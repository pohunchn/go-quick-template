...
TAGS = ""
MOD_NAME = github.com/pohunchn/go-quick-template
...
release: linux-amd64 darwin-amd64 darwin-arm64 windows-x64
	@echo Package go-quick-template
...
.PHONY: linux-amd64
linux-amd64:
	@echo Build go-quick-template [linux-amd64] CGO_ENABLED=$(CGO_ENABLED)
	@CGO_ENABLED=$(CGO_ENABLED) GOOS=linux GOARCH=amd64 go build -trimpath -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(RELEASE_LINUX_AMD64)/$(TARGET)

.PHONY: darwin-amd64
darwin-amd64:
	@echo Build go-quick-template [darwin-amd64] CGO_ENABLED=$(CGO_ENABLED)
...
.PHONY: darwin-arm64
darwin-arm64:
	@echo Build go-quick-template [darwin-arm64] CGO_ENABLED=$(CGO_ENABLED)
...
.PHONY: windows-x64
windows-x64:
	@echo Build go-quick-template [windows-x64] CGO_ENABLED=$(CGO_ENABLED)
